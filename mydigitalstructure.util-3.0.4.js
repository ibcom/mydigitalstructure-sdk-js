		...
		if (param != undefined)
		{
			...
			else // render
			{
				...
				{	
					...
					{
						//app.vq.init('#' +  controller + '-view', {queue: context, clear: true, setDefault: true});
					...
					{	
						...
						if (options.containerController != undefined)
						{
					...
					if (init || options.orientation == 'horizontal')
					{
						...
						app.vq.add('<table class="table table-hover m-b-0"><thead>', {queue: context});
					...
					app.vq.render('#' + controller + '-view', {append: append, queue: context, appendSelector: appendSelector}, data);
