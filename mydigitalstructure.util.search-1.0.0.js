		 			...
		 			'<div class="{{class}} myds-click" data-name="{{name}}" data-controller="{{controller}}">',
						'<div class="ibox">',
							'<div class="ibox-title">',
		                      ...
		                      '{{icon}}',
	                  	'</div>',
								'<h5 class="ml-1">{{caption}}</h5>',
							'</div>',
							'<div class="ibox-content">',
								'<div class="text-muted"><small>{{notes}}</small></div>',
							'</div>',
						'</div>',
					'</div>'
				]
				...
				if (_.isArray(search.userFilters))
				{
					...
					_.each(search.userFilters, function (userFilter)
					{
						...
						[
							'<div class="col-sm-4">',
								'<div>',
	          					'<label class="text-muted mb-1 mt-1 ml-1" for="util-view-search-user-filter-', userFilter.name, '">',
	          						userFilter.caption,
	          					'</label>'
	          		],
	          		{queue: 'util-view-search-show-user-filters'});
					...
					else
					{
						...
						[
							'<div class="col-lg-10">',
								...
								'</div>',
							'</div>',
							'<div class="col-lg-2 pr-3 text-right">',
								'<button id="util-view-search-user-filter-search" style="margin-top:26px;"',
									' class="myds-click btn btn-white" role="button" data-controller="util-view-search-process">',
	            				'Search',
	           				'</button>',
	           			'</div>'
						],
								...
								'<div class="card-header">',
									'<div class="float-right" data-html2canvas-ignore="true">',
				                     ...
				                     '<button class="myds-click btn btn-white btn-sm ml-2" role="button" data-controller="util-view-search-export"',
			              			...
			              			'</div>',
		                  	'</div>',
									'<h3 class="ml-2', (!_.isEmpty(search.notes)?' mb-0':'') , '">{{caption}}</h3>',
									'<div class="ml-2 text-muted"><small>{{notes}}</small></div>',
								'</div>',
